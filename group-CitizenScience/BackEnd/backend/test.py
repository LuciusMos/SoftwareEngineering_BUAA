...
with open('imagenet_classes_ch.txt', encoding='UTF-8') as f:
...
def volunteer_image_ai(project_id, data_title, img_path, api_type="general"):
    ...
    while True:
        ...
        if response:
            ...
            # db.commit()
            # 1)api的this_root不在root_list或this_score<0.1; 2)torch的ch_class不在root_list
            # 若1) 2)同时成立，则认为不合法
            print('baike完毕，准备torch')
            torch_predict_first = predict(img_path, resnet)[0]
    ...
    while True:
        ...
        if response:
            ...
            for result in response.json()['result']:
                ...
                # if this_score > 0.4 and this_root not in data_format[data_name]['root_pool']:
            ...
            # db.commit()
            torch_predict = predict(img_path, resnet)
...
if __name__ == "__main__":
    img_path = "/Users/ivory/Desktop/dog.jpeg"
    api_type = "ingredient"

    f = open(img_path, 'rb')
    print(predict(img_path, resnet))
    # img = base64.b64encode(f.read())
    # params = {"image": img, "baike_num": 5}
