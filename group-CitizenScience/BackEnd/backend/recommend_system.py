...
def branch_recommend(this_project_list, projects):
    ...
    for this_project_id in this_project_list:
        if this_project_id in projects:
...
def get_recommended(user_id):
    ...
    if user_id not in users:
        return hot_list[:20]
    my_project_list = [key for key in users[user_id]]
    ...
    project_list = project_list1 + project_list2 + project_list3 + project_list4 + project_list5
    projects = {}
    ...
    return_list = list(set(return_list).difference(set(my_project_list)))
