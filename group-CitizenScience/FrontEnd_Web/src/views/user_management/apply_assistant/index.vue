<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="申请项目ID" align="center" width="95">
        <template slot-scope="scope">{{ scope.row.project_id }}</template>
      </el-table-column>
      <el-table-column label="申请项目名称" align="center">
        <template slot-scope="scope">{{ scope.row.project_title }}</template>
      </el-table-column>
      <el-table-column align="center" prop="apply_time" label="申请时间" width="150">
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      // params.append("id", this.list.id);
      this.axios
        .post(global.server + "admin/get_data/apply_assistant")
        .then(res => {
    ...
    handleAccept(index, row) {
      let params = new URLSearchParams();
      params.append("id", row.user_id);
      params.append("pid", row.project_id);
      this.axios
        .post(global.server + "admin/upload_data/accept_assistant", params)
        .then(res => {
          if (res.data == "networkError") {
          this.$message({ type: "error", message: "验证过期，请重新登录！" });
          setTimeout(() => {
    ...
    confirmRefuse() {
      ...
      params.append("id", this.chooseRow.user_id);
      params.append("pid", this.chooseRow.project_id);
      this.axios
        .post(global.server + "admin/upload_data/refuse_assistant", params)
        .then(res => {
          if (res.data == "networkError") {
          this.$message({ type: "error", message: "验证过期，请重新登录！" });
          setTimeout(() => {
